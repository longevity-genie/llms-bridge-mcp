...
from llms_bridge_mcp.server import LLMBridgeMCP, LLMBridgeResult
...
async def test_battle_real_llm_calls():
    ...
    if not openai_key and not gemini_key and not perplexity_key:
        pytest.skip("No API keys configured. Set OPENAI_API_KEY, GEMINI_API_KEY, or PERPLEXITY_API_KEY to run battle test")
        ...
        for query, query_result in llm_results.items():
            if query_result["success"]:
                ...
                if "date" in query.lower() and "iso format" in query.lower():
                    ...
                    print(f"✅ {llm_name}: Found current UTC date {current_utc_date} in response")
