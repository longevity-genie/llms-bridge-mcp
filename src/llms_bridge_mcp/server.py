...
class LLMBridgeMCP(FastMCP):
    ...
    async def _call_llm(self, query: str, model: str, **kwargs) -> LLMBridgeResult:
        """
        ...
        try:
            ...
            stream = (
                kwargs.pop("stream", False) and False
            )  # force non-streaming by default
            ...
            if "gemini" not in model:
                completion_kwargs.pop(
                    "web_search_options"
                )  # not supported by non-Gemini models
    ...
    async def call_chatgpt(
        ...
        if not self.openai_api_key:
            return LLMBridgeResult(
                data={}, success=False, message="OPENAI_API_KEY is not configured"
            )
    ...
    async def call_gemini(
        ...
        if not self.gemini_api_key:
            return LLMBridgeResult(
                data={}, success=False, message="GEMINI_API_KEY is not configured"
            )
    ...
    async def call_perplexity(
        ...
        if not self.perplexity_api_key:
            return LLMBridgeResult(
                data={}, success=False, message="PERPLEXITY_API_KEY is not configured"
            )
...
class SmitheryConfigSchema(BaseModel):
    ...
    gemini_api_key: Optional[str] = Field(
        default=None, description="Google Gemini API key (optional)"
    )
    perplexity_api_key: Optional[str] = Field(
        default=None, description="Perplexity API key (optional)"
    )
...
def main(
    ...
    run_with_graceful_shutdown(
        get_mcp_server(), transport=transport, host=host, port=port
    )
